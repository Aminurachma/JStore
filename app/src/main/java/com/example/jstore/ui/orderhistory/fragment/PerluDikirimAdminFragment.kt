...
import com.example.jstore.databinding.FragmentPerluDikirimAdminBinding
...
class PerluDikirimAdminFragment : BaseFragment() {
    private var _binding: FragmentPerluDikirimAdminBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPerluDikirimAdminBinding.inflate(layoutInflater)
        return binding?.root
    }
    ...
    private fun getOrderHistoryList() {
        progress.show()
        FirestoreClass().getOrderHistoryList(Constants.DIKEMAS, onSuccessListener = {
            progress.dismiss()
            if (it.isNotEmpty()) {
