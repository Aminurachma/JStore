...
import com.example.jstore.databinding.FragmentSedangDikirimAdminBinding
...
class SedangDikirimAdminFragment : BaseFragment() {
    private var _binding: FragmentSedangDikirimAdminBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentSedangDikirimAdminBinding.inflate(layoutInflater)
        return binding?.root
    }
    ...
    private fun getOrderHistoryList() {
        progress.show()
        FirestoreClass().getOrderHistoryList(Constants.DIKIRIM, onSuccessListener = {
            progress.dismiss()
            if (it.isNotEmpty()) {
