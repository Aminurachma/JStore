...
import com.example.jstore.databinding.FragmentMenungguKonfirmasiAdminBinding
...
class MenungguKonfirmasiAdminFragment : BaseFragment() {
    private var _binding: FragmentMenungguKonfirmasiAdminBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentMenungguKonfirmasiAdminBinding.inflate(layoutInflater)
        return binding?.root
    }
    ...
    private fun getOrderHistoryList() {
        progress.show()
        FirestoreClass().getOrderHistoryListPayment(Constants.MENUNGGU_KONFIRMASI_ADMIN, onSuccessListener = {
            progress.dismiss()
            if (it.isNotEmpty()) {
